...
class Predictor(threading.Thread):
    ...
    def prepare_sdf(self, force_field: str = "MMFF") -> None:
        """
        ...
        for i in range(len(hm_list)):
            try:
                AllChem.EmbedMolecule(hm_list[i], AllChem.ETKDG())
            except ValueError:
                AllChem.EmbedMolecule(hm_list[i], AllChem.ETKDG(), useRandomCoords=True)

            try:
                if force_field == "MMFF":
                    AllChem.MMFFOptimizeMolecule(hm_list[i], maxIters=1000)
                elif force_field == "UFF":
                    AllChem.UFFOptimizeMolecule(hm_list[i], maxIters=1000)
                writer.write(hm_list[i])
            except ValueError:
                raise ValueError(f"{i} {Chem.MolToSmiles(hm_list[i])}: optimization failed")
        writer.close()
    ...
    def num_arom_ring(self) -> None:
        ...
        """
        num_arom_ring_list = [rdMolDescriptors.CalcNumAromaticRings(m) for m in self.mol_list]

        self.predictions_lock.acquire()
        self.predictions['num_arom_ring'] = num_arom_ring_list
        self.predictions_lock.release()

    def num_rot_bond(self) -> None:
        num_rot_bond_list = [rdMolDescriptors.CalcNumRotatableBonds(m) for m in self.mol_list]

        self.predictions_lock.acquire()
        self.predictions['num_rot_bond'] = num_rot_bond_list
        self.predictions_lock.release()
    ...
    def _prepare_docking_file(self, force_field: str = "MMFF") -> None:
        # prepare .pdbqt files
        ...
        if self.dock_method == "vina":
            ...
            for i in range(len(self.mols)):
                ...
                if force_field == "MMFF":
                    AllChem.MMFFOptimizeMolecule(hmol, maxIters=1000)
                elif force_field == "UFF":
                    AllChem.UFFOptimizeMolecule(hmol, maxIters=1000)
                hmol.SetProp("smiles", f"{self.mols[i]} {i}")
                writer.write(hmol)
            writer.close()
